describe('Breaker Component', () => {
  beforeEach(() => {
    cy.visit('iframe.html?args=&id=editorial-breaker--left&viewMode=story');
  });

  it('should render the Left variant correctly', () => {
    // Check for headline
    cy.get('.breaker .row').within(() => {
    ...
    cy.get('.breaker .shadow img').should('have.attr', 'src', './images/card.webp');

    // Check for body text
    cy.contains('Contra legem facit qui id facit quod lex prohibet.');

    // Check for button
    cy.get('.breaker .btn-primary').contains('Call to action');
  });

  it('should render the Right variant correctly', () => {
    cy.visit('/iframe.html?args=&id=editorial-breaker--right&viewMode=story');
    ...
    cy.get('.breaker .row').should('have.css', 'flex-direction', 'row-reverse');
  });
  ...
  context('Responsive Design Tests', () => {
    ['ipad-2', 'iphone-6+', 'macbook-15'].forEach((size) => {
      it(`should display correctly on ${size} screen`, () => {
        ...
        cy.get('.breaker .shadow img')
          .should('have.attr', 'src', './images/card.webp');
        cy.contains('Contra legem facit qui id facit quod lex prohibet.')
          .should('be.visible');
        cy.get('.breaker .btn-primary')
